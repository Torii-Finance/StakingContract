...
contract StakeEarn is Ownable, ReentrancyGuard {
    ...
    function initialize(uint256 _rewardPerStake, uint256 _startTime, uint256 _endTime, uint256 _minStakePerUser, uint256 _maxStakePerUser, uint256 _poolStakeLimit, uint256 _minTimeToStake) external onlyOwner{
    ...
    function deposit(uint256 _amount) external nonReentrant {
		...
		uint256 _taxAmount = _amount.mul(tax).div(100);
		uint256 _finalAmount = _amount.sub(_taxAmount);
		if (_amount > 0) 
		{
             user.amount = _finalAmount;
		...
		limitLeft = limitLeft.sub(_finalAmount);
        emit Deposit(msg.sender, _finalAmount);
    }
